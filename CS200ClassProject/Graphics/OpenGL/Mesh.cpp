...
namespace Graphics
{
    ...
    std::size_t Mesh::GetPointCount() const noexcept
    {
     ...
     vector2<float> Graphics::Mesh::GetPoint(std::size_t index) const noexcept
    {
    ...
    Color4ub Graphics::Mesh::GetColor(std::size_t index) const noexcept 
    {
     ...
     vector2<float> Graphics::Mesh::GetTextureCoordinate(std::size_t index) const noexcept
    {
        if (textureCoordinates.empty())
        {
            return  vector2<float>{0.f};
        }
    ...
    void Graphics::Mesh::AddPoint( vector2<float> point) noexcept
    { points.push_back(point);
    }
    void Graphics::Mesh::AddTextureCoordinate( vector2<float> texture_coordinate) noexcept
    {
    ...
    { 
        ...
        circle.AddColor(color);
         ...
         circle.AddPoint( vector2<float>{0.f});
        ...
        {
            circle.AddPoint(
                 vector2<float>{radius * cos((temp * count) - MATH::HALF_PI), radius * sin((temp * count) - MATH::HALF_PI)});
        }
    ...
    Mesh MESH::create_line( vector2<float> a,  vector2<float> b, Color4ub color) noexcept
    { 
        Mesh line;
